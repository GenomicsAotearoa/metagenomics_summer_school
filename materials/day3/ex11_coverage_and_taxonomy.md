...
For an example of one way in which the `cov_table.txt` output generated by `jgi_summarize_bam_contig_depths` above could then be normalised based on average library size, see the [Normalise per-sample bin coverage values](#optional-normalise-persample-bin-coverage-values-by-average-library-size) section below.
...
The script `norm_jgi_cov_table.py` is available in the folder `8.coverage_and_taxonomy/`, and is also available for download for future reference at [this link](https://github.com/GenomicsAotearoa/metagenomics_summer_school/blob/master/materials/resources/norm_jgi_cov_table.py). 
...
Run `norm_jgi_cov_table.py` for microbial bins data and viral contigs, inputting: A. the `bins_cov_table.txt` and `mapping_filtered_bins.err` files; and B. the `viruses_cov_table.txt` and `mapping_filtered_viruses.err` files. This will generate the outputs `normalised_bins_cov_table.txt` and `normalised_bins_cov_table.txt`. 
