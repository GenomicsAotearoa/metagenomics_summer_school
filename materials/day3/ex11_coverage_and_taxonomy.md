...
For an example of one way in which the `cov_table.txt` output generated by `jgi_summarize_bam_contig_depths` above could then be normalised based on average library size, see the [Normalise per-sample bin coverage values](#optional-normalise-per-sample-coverage-values-by-average-library-size) section below.
