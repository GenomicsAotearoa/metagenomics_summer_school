...
The only *required* input file for `VizBin` is a single `.fna` file of the concatenated bins. An additional annotation file containing per-contig coverage values and bin IDs can also be provided, as colouring contigs by bin is a really effective way to spot areas that might need refinement.

In the interests of time today, the input files have been generated and are provided in the `6.bin_refinement` folder: 

* `all_bins.fna` is a concatenation of the bins of *fragmented* contigs (fragmented to 20k)
* `all_bins.sample1.vizbin.ann` is the annotation file containing per-subcontig coverage, label (bin ID), and length values.

For future reference, and for working with your own data, the step-by-step process for generating these files from the output bins generated by `DAS_Tool` has been provided as an appendix at the end of this page.

We will first create a few variations of the *.ann* file with various columns removed, in order to examine the different outputs they can generate.
