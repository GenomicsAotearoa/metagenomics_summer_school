...
For an example of one way in which the `cov_table.txt` output generated by `jgi_summarize_bam_contig_depths` above could then be normalised based on average library size, see the [Normalise per-sample coverage Appendix](../resources//APPENDIX_ex11_Normalise_coverage_example.md).
